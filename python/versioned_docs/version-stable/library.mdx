...
## Интерактивный режим (REPL) {#interactive-mode-repl}
...
### `time.sleep()` приводит к устаревшему состоянию {#timesleep-leads-to-outdated-state}
...
### несовместимость с `SelectorEventLoop` из `asyncio` на Windows {#incompatible-with-selectoreventloop-of-asyncio-on-windows}
