...
**Настройка опций фикстур**: для фикстур `browser` и `context` используйте следующие фикстуры, чтобы задавать пользовательские опции запуска.
- `browser_type_launch_args`: переопределяет аргументы запуска для [browser_type.launch()](/api/class-browsertype.mdx#browser-type-launch). Должна возвращать Dict.
- `browser_context_args`: переопределяет опции для [browser.new_context()](/api/class-browser.mdx#browser-new-context). Должна возвращать Dict.
- `connect_options`: подключается к существующему браузеру через WebSocket endpoint. Должна возвращать Dict с опциями [browser_type.connect()](/api/class-browsertype.mdx#browser-type-connect).
...
## Параллелизм: Запуск нескольких тестов одновременно {#parallelism-running-multiple-tests-at-once}
...
### Настройка типов для автозаполнения {#configure-typings-for-auto-completion}
...
### Использование нескольких контекстов {#using-multiple-contexts}
...
### Пропуск теста для определенного браузера {#skip-test-by-browser}
...
### Запуск на конкретном браузере {#run-on-a-specific-browser}
...
### Запуск с использованием пользовательского канала браузера, например, Google Chrome или Microsoft Edge {#run-with-a-custom-browser-channel-like-google-chrome-or-microsoft-edge}
...
### Настройка базового URL {#configure-base-url}
...
### Игнорирование ошибок HTTPS {#ignore-https-errors}
...
### Использование пользовательского размера окна просмотра {#use-custom-viewport-size}
...
### Эмуляция устройства / Переопределение параметров BrowserContext {#device-emulation-browsercontext-option-overrides}
...
### Подключение к удалённым браузерам {#connect-to-remote-browsers}
...
### Использование с `unittest.TestCase` {#using-with-unittesttestcase}
...
### Использование с pdb {#use-with-pdb}
...
## Развертывание в CI {#deploy-to-ci}

Смотрите [руководства для провайдеров CI](./ci.mdx), чтобы развернуть ваши тесты в CI/CD.

## Асинхронные фикстуры {#async-fixtures}

Чтобы использовать асинхронные фикстуры, установите [`pytest-playwright-asyncio`](https://pypi.org/project/pytest-playwright-asyncio/).

Убедитесь, что вы используете `pytest-asyncio>=0.26.0`, и задайте [`asyncio_default_test_loop_scope = session`](https://pytest-asyncio.readthedocs.io/en/v0.26.0/how-to-guides/change_default_test_loop.html) в конфигурации (`pytest.ini/pyproject.toml/setup.cfg`).
