...
:::warning[Проверка состояния «отмечено»]
...
:::warning[Проверка отключенного состояния]
...
:::warning[Проверка редактируемого состояния]
...
:::warning[Проверка состояния «включено»]
...
Возвращает описание локатора, ранее установленное с помощью [locator.describe()](/api/class-locator.mdx#locator-describe). Возвращает `null`, если пользовательское описание не было задано. Для человекочитаемого представления предпочтительнее использовать `Locator.toString()`, так как он использует описание, когда оно доступно.
