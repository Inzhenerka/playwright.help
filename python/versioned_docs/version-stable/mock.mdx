...
## Модификация ответов API {#modify-api-responses}
...
В трассировке нашего теста мы можем увидеть, что API был вызван и ответ был изменен. ![трассировка теста, показывающая вызов API и выполнение (fulfill)](https://github.com/microsoft/playwright/assets/13063165/8b8dd82d-1b3e-428e-871b-840581fed439)

При проверке ответа мы можем увидеть, что наш новый фрукт был добавлен в список. ![трассировка теста, показывающая мок-ответ](https://github.com/microsoft/playwright/assets/13063165/03e6c87c-4ecc-47e8-9ca0-30fface25e9d)

Подробнее о [продвинутой работе с сетью](./network.mdx).

## Мокирование с использованием HAR-файлов {#mocking-with-har-files}
...
#### 
В качестве альтернативы вы также можете записывать файлы HAR, используя параметр [record_har_path](/api/class-browser.mdx#browser-new-context-option-record-har-path) в [browser.new_context()](/api/class-browser.mdx#browser-new-context) при создании контекста браузера. Это позволяет захватывать весь сетевой трафик для всего контекста до его закрытия.
