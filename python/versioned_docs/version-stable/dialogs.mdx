...
Слушатель [page.on("dialog")](/api/class-page.mdx#page-event-dialog) **должен обработать** диалог. В противном случае ваше действие будет приостановлено, будь то [locator.click()](/api/class-locator.mdx#locator-click) или что-то еще. Это потому, что диалоги в вебе являются модальными и, следовательно, блокируют дальнейшее выполнение страницы, пока они не будут обработаны.
:::
...
page.get_by_role("button").click() # Здесь произойдет зависание
```
...
```py
page.on("dialog", lambda dialog: print(dialog.message))
await page.get_by_role("button").click() # Здесь произойдет зависание
```
