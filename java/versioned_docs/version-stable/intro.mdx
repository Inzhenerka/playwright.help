...
Playwright распространяется в виде набора модулей [Maven](https://maven.apache.org/what-is-maven.html). Самый простой способ использовать его — добавить одну зависимость в `pom.xml` вашего проекта, как описано ниже. Если вы не знакомы с Maven, пожалуйста, обратитесь к его [документации](https://maven.apache.org/guides/getting-started/maven-in-five-minutes.html).
...
Запуск загружает пакет Playwright и устанавливает бинарные файлы браузеров для Chromium, Firefox и WebKit. Чтобы изменить это поведение, смотрите [параметры установки](./browsers.mdx#install-browsers).

## Первый скрипт {#first-script}
...
По умолчанию Playwright запускает браузеры в безголовом режиме. Чтобы увидеть интерфейс браузера, установите опцию [setHeadless](/api/class-browsertype.mdx#browser-type-launch-option-headless) в `false`. Вы также можете использовать [setSlowMo](/api/class-browsertype.mdx#browser-type-launch-option-slow-mo) для замедления выполнения. Узнайте больше в разделе инструментов отладки [section](./debug.mdx).

```java
playwright.firefox().launch(new BrowserType.LaunchOptions().setHeadless(false).setSlowMo(50));
```

## Запуск скрипта примера {#running-the-example-script}
...
- [Генерируйте тесты с помощью Codegen](./codegen.mdx)
