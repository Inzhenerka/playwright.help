---
id: touch-events
title: "События касания (устарело)"
---
...
Веб-приложения, которые обрабатывают устаревшие [события касания](https://developer.mozilla.org/en-US/docs/Web/API/Touch_events) для реагирования на жесты, такие как свайп, сжатие и нажатие, могут быть протестированы путем ручной отправки [TouchEvent](https://developer.mozilla.org/en-US/docs/Web/API/TouchEvent/TouchEvent) на страницу. Примеры ниже демонстрируют, как использовать [Locator.dispatchEvent()](/api/class-locator.mdx#locator-dispatch-event) и передавать [Touch](https://developer.mozilla.org/en-US/docs/Web/API/Touch) точки в качестве аргументов.
...
### Эмуляция жеста панорамирования {#emulating-pan-gesture}

В примере ниже мы эмулируем жест панорамирования, который должен перемещать карту. Приложение, которое тестируется, использует только координаты `clientX/clientY` точки касания, поэтому мы инициализируем только их. В более сложном сценарии вам может понадобиться также установить `pageX/pageY/screenX/screenY`, если ваше приложение их использует.
...
### Эмуляция жеста сжатия {#emulating-pinch-gesture}

В примере ниже мы эмулируем жест сжатия, то есть две точки касания, которые движутся ближе друг к другу. Ожидается, что это уменьшит масштаб карты. Приложение, которое тестируется, использует только координаты `clientX/clientY` точек касания, поэтому мы инициализируем только их. В более сложном сценарии вам может понадобиться также установить `pageX/pageY/screenX/screenY`, если ваше приложение их использует.
