...
API для сбора и сохранения трассировок Playwright. Трассировки Playwright можно открыть в [Trace Viewer](../trace-viewer.mdx) после выполнения Playwright-скрипта.

:::note

Скорее всего, вместо использования `context.tracing` вам нужно [включить трассировку в конфигурационном файле](https://playwright.dev/docs/api/class-testoptions#test-options-trace).

API `context.tracing` записывает операции браузера и сетевую активность, но не фиксирует проверки в тестах (например, вызовы `expect`). Мы рекомендуем [включать трассировку через конфигурацию Playwright Test](https://playwright.dev/docs/api/class-testoptions#test-options-trace): она включает эти проверки и даёт более полную трассу для отладки падений тестов.
:::
...
Скорее всего, вам стоит [включить трассировку в конфиге](https://playwright.dev/docs/api/class-testoptions#test-options-trace), а не использовать `Tracing.start`.

API `context.tracing` записывает действия в браузере и сетевую активность, но не включает проверки (assertions) теста, например вызовы `expect`. Мы рекомендуем [включать трассировку через конфигурацию Playwright Test](https://playwright.dev/docs/api/class-testoptions#test-options-trace): тогда в трассировку попадут и проверки, и у вас будет более полный трейс для отладки падений тестов.
:::
