...
#### 
В качестве альтернативы вы также можете записывать HAR-файлы, используя опцию [recordHar](/api/class-browser.mdx#browser-new-context-option-record-har) в [browser.newContext()](/api/class-browser.mdx#browser-new-context) при создании контекста браузера. Это позволяет захватить весь сетевой трафик для всего контекста до тех пор, пока контекст не будет закрыт.
