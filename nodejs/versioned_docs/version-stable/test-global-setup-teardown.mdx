...
Есть два способа настроить global setup и teardown: использовать файл global setup и указать его в конфиге через [`globalSetup`](#option-2-configure-globalsetup-and-globalteardown), либо использовать [зависимости проектов](#option-1-project-dependencies). При использовании зависимостей проектов вы определяете проект, который запускается перед всеми остальными проектами. Это рекомендуемый подход, так как он лучше интегрируется с раннером Playwright: global setup будет виден в HTML-отчёте, будут записываться трассы, и можно будет использовать фикстуры. Подробное сравнение двух подходов — в таблице ниже.

| Возможность | Зависимости проектов (рекомендуется) | `globalSetup` (опция конфига) |
|---|---|---|
...
## Вариант 1: Зависимости проекта {#option-1-project-dependencies}
...
### Фильтрация тестов {#test-filtering}

Все опции фильтрации тестов, такие как `--grep`/`--grep-invert`, `--shard`, фильтрация напрямую по позиции (location) в командной строке или использование [`test.only()`](./api/class-test.mdx#test-only), напрямую выбирают основные тесты, которые будут запущены. Если эти тесты относятся к проекту с зависимостями, то будут запущены и все тесты из этих зависимостей.
...
## Вариант 2: Настройка globalSetup и globalTeardown {#option-2-configure-globalsetup-and-globalteardown}
...
Учтите, что у `globalSetup` и `globalTeardown` не хватает некоторых возможностей — подробное сравнение см. в разделе [введение](#introduction). Вместо этого рассмотрите использование [зависимостей проектов](#option-1-project-dependencies), чтобы получить поддержку всех возможностей.
:::
...
### Захват трассировки ошибок во время глобальной настройки {#capturing-trace-of-failures-during-global-setup}
