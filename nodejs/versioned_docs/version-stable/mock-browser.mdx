...
## Мокирование только для чтения API {#mocking-read-only-apis}
...
## Проверка вызовов API {#verifying-api-calls}
