...
## Генерация скриншотов {#generating-screenshots}
...
> Обратите внимание: `toHaveScreenshot()` также принимает массив сегментов пути к файлу снапшота, например `expect().toHaveScreenshot(['relative', 'path', 'to', 'snapshot.png'])`.
> Однако этот путь должен оставаться внутри директории снапшотов для каждого файла теста (т. е. `a.spec.js-snapshots`), иначе будет выброшена ошибка.

## Обновление скриншотов {#updating-screenshots}
...
## Снимки не изображений {#non-image-snapshots}
