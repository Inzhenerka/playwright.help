...
Файлы видео будут появляться в каталоге вывода тестов, обычно это `test-results`. См. [testOptions.video](/api/class-testoptions.mdx#test-options-video) для расширенной конфигурации видео.

Видео сохраняются при закрытии [контекста браузера](./browser-contexts.mdx) в конце теста. Если вы создаете контекст браузера вручную, убедитесь, что вы дожидаетесь завершения [browserContext.close()](/api/class-browsercontext.mdx#browser-context-close).
...
Для сценариев с несколькими страницами вы можете получить доступ к файлу видео, связанному со страницей, через [page.video()](/api/class-page.mdx#page-video).
