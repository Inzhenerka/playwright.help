...
### Опции эмуляции {#emulation-options}
...
### Дополнительные опции браузера и контекста {#more-browser-and-context-options}
...
### Явное создание контекста и наследование опций {#explicit-context-creation-and-option-inheritance}
...
### Области конфигурации {#configuration-scopes}
...
### Сбросить опцию {#reset-an-option}

Вы можете сбросить опцию к значению, заданному в файле конфигурации. Рассмотрим следующую конфигурацию, которая задаёт `baseURL`:
...
Теперь вы можете настроить `baseURL` для файла, а также отключить его для одного теста.

```js title="intro.spec.ts"
import { test } from '@playwright/test';

// Настройте baseURL для этого файла.
test.use({ baseURL: 'https://playwright.dev/docs/intro' });

test('check intro contents', async ({ page }) => {
  // Этот тест будет использовать базовый URL "https://playwright.dev/docs/intro", заданный выше.
});

test.describe(() => {
  // Сбросьте значение к тому, что задано в конфигурации.
  test.use({ baseURL: undefined });

  test('can navigate to intro from the home page', async ({ page }) => {
    // Этот тест будет использовать базовый URL "https://playwright.dev", заданный в конфигурации.
  });
});
```

Если вы хотите полностью сбросить значение до `undefined`, используйте развёрнутую нотацию фикстуры.

```js title="intro.spec.ts"
import { test } from '@playwright/test';

// Полностью отключите baseURL для этого файла.
test.use({
