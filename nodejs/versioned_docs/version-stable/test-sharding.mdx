...
## Шардинг тестов между несколькими машинами {#sharding-tests-between-multiple-machines}
...
## Балансировка шардов {#balancing-shards}
...
## Объединение отчетов из нескольких шардов {#merging-reports-from-multiple-shards}
...
Blob-отчёт содержит информацию обо всех запущенных тестах и их результатах, а также все вложения тестов — например, трассы и диффы скриншотов. Blob-отчёты можно объединять и конвертировать в любой другой отчёт Playwright. По умолчанию blob-отчёт будет сгенерирован в директорию `blob-report`. Подробнее об опциях blob-отчёта см. [здесь](./test-reporters.mdx#blob-reporter).
...
## Пример для GitHub Actions {#github-actions-example}
...
## Объединение отчётов из разных окружений {#merging-reports-from-multiple-environments}

Если вы хотите запускать одни и те же тесты в нескольких окружениях (а не шардить тесты между несколькими машинами), эти окружения нужно различать.

В этом случае полезно задать свойство [testConfig.tag](/api/class-testconfig.mdx#test-config-tag), чтобы пометить все тесты именем окружения. Этот тег будет автоматически подхвачен blob-отчётом, а затем и инструментом объединения.
...
## CLI для объединения отчетов {#merge-reports-cli}
