...
Тестовый раннер уведомляет репортер о различных событиях во время выполнения тестов. Все методы репортера являются необязательными.

Вы можете создать пользовательский репортер, реализовав класс с некоторыми методами репортера. Убедитесь, что вы экспортируете этот класс по умолчанию.
...
class MyReporter implements Reporter {
  constructor(options: { customOption?: string } = {}) {
    console.log(`my-awesome-reporter настроен с customOption равным ${options.customOption}`);
  }
  ...
  onTestEnd(test: TestCase, result: TestResult) {
    console.log(`Завершен тест ${test.title}: ${result.status}`);
  }

  onEnd(result: FullResult) {
    console.log(`Завершен запуск: ${result.status}`);
  }
}
...
* [reporter.onBegin()](/api/class-reporter.mdx#reporter-on-begin) вызывается один раз с корневым набором, который содержит все остальные наборы и тесты. Узнайте больше о [иерархии наборов][Suite].
* [reporter.onTestBegin()](/api/class-reporter.mdx#reporter-on-test-begin) вызывается для каждого запуска теста. Ему передается [TestCase], который выполняется, и [TestResult], который почти пуст. Результат теста будет заполняться во время выполнения теста (например, шагами и стандартным выводом) и получит окончательный `status`, как только тест завершится.
...
* [reporter.onEnd()](/api/class-reporter.mdx#reporter-on-end) вызывается один раз после завершения всех тестов, которые должны были выполниться.
* [reporter.onExit()](/api/class-reporter.mdx#reporter-on-exit) вызывается немедленно перед выходом тестового раннера.

Кроме того, [reporter.onStdOut()](/api/class-reporter.mdx#reporter-on-std-out) и [reporter.onStdErr()](/api/class-reporter.mdx#reporter-on-std-err) вызываются, когда стандартный вывод производится в рабочем процессе, возможно, во время выполнения теста, а [reporter.onError()](/api/class-reporter.mdx#reporter-on-error) вызывается, когда что-то пошло не так вне выполнения теста.
...
**Примечания по API объединенного отчета**

При объединении нескольких [`blob`](../test-reporters#blob-reporter) отчетов через команду CLI [`merge-reports`](../test-sharding#merge-reports-cli) вызывается тот же [Reporter] API для создания окончательных отчетов, и все существующие репортеры должны работать без каких-либо изменений. Однако есть некоторые тонкие различия, которые могут повлиять на некоторые пользовательские репортеры.
* Проекты из разных шардов всегда остаются отдельными [TestProject] объектами. Например, если проект 'Desktop Chrome' был разделен на 5 машин, то в конфигурации, переданной в [reporter.onBegin()](/api/class-reporter.mdx#reporter-on-begin), будет 5 экземпляров проектов с одинаковым именем.
...
Вызывается один раз перед запуском тестов. Все тесты уже были обнаружены и помещены в иерархию [Suite]ов.
...
- `suite` [Suite]<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="reporter-on-begin-option-suite"/><a href="#reporter-on-begin-option-suite" class="list-anchor">#</a>
  ...
  Корневой набор, который содержит все проекты, файлы и тестовые случаи.
...
Вызывается после того, как все тесты были выполнены, или тестирование было прервано. Обратите внимание, что этот метод может возвращать [Promise], и Playwright Test будет ожидать его. Репортер может переопределить статус и, таким образом, повлиять на код выхода тестового раннера.
...
- [Promise]&lt;[Object]&gt;<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="reporter-on-end-return"/><a href="#reporter-on-end-return" class="list-anchor">#</a>
  - `status` "passed" | "failed" | "timedout" | "interrupted" *(необязательно)*
...
Вызывается при возникновении глобальной ошибки, например, необработанного исключения в рабочем процессе.
...
Вызывается немедленно перед выходом тестового раннера. На этом этапе все репортеры получили сигнал [reporter.onEnd()](/api/class-reporter.mdx#reporter-on-end), поэтому все отчеты должны быть собраны. Вы можете выполнить код, который загружает отчеты в этом хуке.
...
Вызывается, когда что-то было записано в стандартный поток ошибок в рабочем процессе.
...
- `test` [void] | [TestCase]<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="reporter-on-std-err-option-test"/><a href="#reporter-on-std-err-option-test" class="list-anchor">#</a>
  ...
  Тест, который выполнялся. Обратите внимание, что вывод может происходить, когда ни один тест не выполняется, в этом случае это будет [void].
...
Вызывается, когда что-то было записано в стандартный поток вывода в рабочем процессе.
...
Вызывается, когда шаг теста начался в рабочем процессе.
...
- `test` [TestCase]<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="reporter-on-step-begin-option-test"/><a href="#reporter-on-step-begin-option-test" class="list-anchor">#</a>
  ...
  Тест, к которому принадлежит шаг.
...
Вызывается, когда шаг теста завершен в рабочем процессе.
...
Вызывается после того, как тест был запущен в рабочем процессе.
...
Вызывается после того, как тест был завершен в рабочем процессе.
...
Указывает, использует ли этот репортер стандартный ввод-вывод для отчетности. Если он этого не делает, Playwright Test может добавить некоторый вывод для улучшения пользовательского опыта. Если ваш репортер не выводит в терминал, настоятельно рекомендуется вернуть `false`.
