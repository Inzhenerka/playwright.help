...
Заголовок `Cookie` нельзя переопределить с помощью этого метода. Если значение указано, оно будет проигнорировано, а cookies будут загружены из хранилища cookies браузера. Чтобы задать свои cookies, используйте [browserContext.addCookies()](/api/class-browsercontext.mdx#browser-context-add-cookies).
:::
