...
Когда страница отправляет запрос на сетевой ресурс, [Page] генерирует следующую последовательность событий:
...
Если запрос не удается на каком-то этапе, то вместо события `'requestfinished'` (и, возможно, вместо события 'response') генерируется событие [page.on('requestfailed')](/api/class-page.mdx#page-event-request-failed).
...
Содержит тип ресурса запроса, как он воспринимается движком рендеринга. ResourceType будет одним из следующих: `document`, `stylesheet`, `image`, `media`, `font`, `script`, `texttrack`, `xhr`, `fetch`, `eventsource`, `websocket`, `manifest`, `other`.
...
[Worker] Service, который выполняет запрос.
...
- [Object]<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="request-timing-return"/><a href="#request-timing-return" class="list-anchor">#</a>
  ...
  - `secureConnectionStart` [number]
    ...
    Время непосредственно перед тем, как браузер начинает процесс рукопожатия для обеспечения текущего соединения. Значение указано в миллисекундах относительно `startTime`, -1, если недоступно.
  - `connectEnd` [number]
