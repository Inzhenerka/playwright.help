...
Нужно ли завершать выполнение с ошибкой, если какие‑либо тесты помечены как flaky. Полезно в CI.

Также доступно в [командной строке](../test-cli.mdx) через параметр `--fail-on-flaky-tests`.
...
Тег или теги, добавляемые в начало каждого теста в отчёте. Полезно, чтобы помечать запуск тестов и различать [CI-окружения](../test-sharding.mdx#merging-reports-from-multiple-environments).

Обратите внимание: каждый тег должен начинаться с символа `@`. Подробнее о [тегировании](../test-annotations.mdx#tag-tests).
