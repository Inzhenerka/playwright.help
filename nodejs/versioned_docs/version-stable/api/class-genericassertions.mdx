...
Класс [GenericAssertions] предоставляет методы утверждений, которые можно использовать для проверки любых значений в тестах. Новый экземпляр [GenericAssertions] создается вызовом [expect()](/api/class-playwrightassertions.mdx#playwright-assertions-expect-generic):

```js
import { test, expect } from '@playwright/test';

test('утверждение значения', async ({ page }) => {
  const value = 1;
  expect(value).toBe(2);
});
```
...
- `constructor` [Function]<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="generic-assertions-any-option-constructor"/><a href="#generic-assertions-any-option-constructor" class="list-anchor">#</a>
  ...
  Конструктор ожидаемого объекта, например `ExampleClass`, или примитивный обернутый тип, такой как `Number`.
...
`expect.anything()` соответствует всему, кроме `null` и `undefined`. Используйте его внутри [expect(value).toEqual()](/api/class-genericassertions.mdx#generic-assertions-to-equal) для выполнения сопоставления по шаблону.
...
`expect.arrayContaining()` соответствует массиву, который содержит все элементы ожидаемого массива в любом порядке. Обратите внимание, что полученный массив может быть надмножеством ожидаемого массива и содержать некоторые дополнительные элементы.

Используйте этот метод внутри [expect(value).toEqual()](/api/class-genericassertions.mdx#generic-assertions-to-equal) для выполнения сопоставления по шаблону.
...
Сравнивает числа с плавающей запятой на предмет приблизительного равенства. Используйте этот метод внутри [expect(value).toEqual()](/api/class-genericassertions.mdx#generic-assertions-to-equal) для выполнения сопоставления по шаблону. При сравнении только двух чисел предпочтительнее использовать [expect(value).toBeCloseTo()](/api/class-genericassertions.mdx#generic-assertions-to-be-close-to).
...
- `numDigits` [number] *(опционально)*<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="generic-assertions-close-to-option-num-digits"/><a href="#generic-assertions-close-to-option-num-digits" class="list-anchor">#</a>
  ...
  Количество десятичных знаков после запятой, которые должны быть равны.
...
`expect.objectContaining()` соответствует объекту, который содержит и соответствует всем свойствам ожидаемого объекта. Обратите внимание, что полученный объект может быть надмножеством ожидаемого объекта и содержать некоторые дополнительные свойства.

Используйте этот метод внутри [expect(value).toEqual()](/api/class-genericassertions.mdx#generic-assertions-to-equal) для выполнения сопоставления по шаблону. Свойства объекта могут быть матчерами для дальнейшего ослабления ожидания. См. примеры.
...
// Матчеры также могут использоваться на свойствах.
...
- `expected` [Object]<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="generic-assertions-object-containing-option-expected"/><a href="#generic-assertions-object-containing-option-expected" class="list-anchor">#</a>
  ...
  Ожидаемый объект, который содержит подмножество свойств.
...
`expect.stringContaining()` соответствует строке, которая содержит ожидаемую подстроку. Используйте этот метод внутри [expect(value).toEqual()](/api/class-genericassertions.mdx#generic-assertions-to-equal) для выполнения сопоставления по шаблону.
...
`expect.stringMatching()` соответствует полученной строке, которая в свою очередь соответствует ожидаемому шаблону. Используйте этот метод внутри [expect(value).toEqual()](/api/class-genericassertions.mdx#generic-assertions-to-equal) для выполнения сопоставления по шаблону.
...
Сравнивает значение с [ожидаемым](/api/class-genericassertions.mdx#generic-assertions-to-be-option-expected), вызывая `Object.is`. Этот метод сравнивает объекты по ссылке, а не по их содержимому, аналогично оператору строгого равенства `===`.
...
Сравнивает числа с плавающей запятой на предмет приблизительного равенства. Используйте этот метод вместо [expect(value).toBe()](/api/class-genericassertions.mdx#generic-assertions-to-be) при сравнении чисел с плавающей запятой.
...
Гарантирует, что значение не является `undefined`.
...
Гарантирует, что значение является ложным в логическом контексте, одним из `false`, `0`, `''`, `null`, `undefined` или `NaN`. Используйте этот метод, когда вам не важен конкретный результат.
...
Гарантирует, что `value > expected` для числовых или больших целочисленных значений.
...
Гарантирует, что значение является экземпляром класса. Использует оператор `instanceof`.
...
Гарантирует, что значение является `NaN`.
...
Гарантирует, что значение является `null`.
...
Гарантирует, что значение истинно в логическом контексте, **все, кроме** `false`, `0`, `''`, `null`, `undefined` или `NaN`. Используйте этот метод, когда вам не важен конкретный результат.
...
Гарантирует, что значение является `undefined`.
...
Гарантирует, что строковое значение содержит ожидаемую подстроку. Сравнение регистронезависимо.
...
Гарантирует, что значение является `Array` или `Set` и содержит ожидаемый элемент.
...
Гарантирует, что значение является `Array` или `Set` и содержит элемент, равный ожидаемому.
...
Сравнивает содержимое значения с содержимым [ожидаемого](/api/class-genericassertions.mdx#generic-assertions-to-equal-option-expected), выполняя проверку "глубокого равенства".
...
**Не строгая равенство**

[expect(value).toEqual()](/api/class-genericassertions.mdx#generic-assertions-to-equal) выполняет проверку глубокого равенства, которая сравнивает содержимое полученных и ожидаемых значений. Чтобы убедиться, что два объекта ссылаются на один и тот же экземпляр, используйте [expect(value).toBe()](/api/class-genericassertions.mdx#generic-assertions-to-be) вместо этого.

[expect(value).toEqual()](/api/class-genericassertions.mdx#generic-assertions-to-equal) игнорирует свойства и элементы массива, равные `undefined`, и не настаивает на равенстве типов объектов. Для более строгого сопоставления используйте [expect(value).toStrictEqual()](/api/class-genericassertions.mdx#generic-assertions-to-strict-equal).

**Сопоставление по шаблону**

[expect(value).toEqual()](/api/class-genericassertions.mdx#generic-assertions-to-equal) также можно использовать для выполнения сопоставления по шаблону на объектах, массивах и примитивных типах с помощью следующих матчеров:
...
Вот пример, который утверждает некоторые значения внутри сложного объекта:
...
Гарантирует, что значение имеет свойство `.length`, равное [ожидаемому](/api/class-genericassertions.mdx#generic-assertions-to-have-length-option-expected). Полезно для массивов и строк.
...
Гарантирует, что свойство по указанному `keyPath` существует в объекте и при необходимости проверяет, что свойство равно [ожидаемому](/api/class-genericassertions.mdx#generic-assertions-to-have-property-option-expected). Равенство проверяется рекурсивно, аналогично [expect(value).toEqual()](/api/class-genericassertions.mdx#generic-assertions-to-equal).
...
- `expected` [Object] *(опционально)*<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="generic-assertions-to-have-property-option-expected"/><a href="#generic-assertions-to-have-property-option-expected" class="list-anchor">#</a>
  ...
  Необязательное ожидаемое значение для сравнения с свойством.
...
Гарантирует, что строковое значение соответствует регулярному выражению.
...
Сравнивает содержимое значения с содержимым [ожидаемого](/api/class-genericassertions.mdx#generic-assertions-to-match-object-option-expected), выполняя проверку "глубокого равенства". Позволяет присутствовать дополнительным свойствам в значении, в отличие от [expect(value).toEqual()](/api/class-genericassertions.mdx#generic-assertions-to-equal), так что вы можете проверить только подмножество свойств объекта.
...
Сравнивает содержимое значения с содержимым [ожидаемого](/api/class-genericassertions.mdx#generic-assertions-to-strict-equal-option-expected) **и** их типами.
...
* Проверяется разреженность массива. Например, `[, 1]` не соответствует `[undefined, 1]`.
...
Вызывает функцию и гарантирует, что она выбрасывает ошибку.

При необходимости сравнивает ошибку с [ожидаемым](/api/class-genericassertions.mdx#generic-assertions-to-throw-option-expected). Допустимые ожидаемые значения:
...
Псевдоним для [expect(value).toThrow()](/api/class-genericassertions.mdx#generic-assertions-to-throw).
...
Заставляет утверждение проверять противоположное условие. Например, следующий код проходит:
