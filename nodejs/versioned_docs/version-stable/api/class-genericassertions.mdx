...
`expect.arrayOf()` сопоставляет массив объектов, созданных с помощью [constructor](/api/class-genericassertions.mdx#generic-assertions-array-of-option-constructor), или соответствующего примитивного типа. Используйте его внутри [expect(value).toEqual()](/api/class-genericassertions.mdx#generic-assertions-to-equal), чтобы выполнить сопоставление по шаблону.
...
- `constructor` [Function]<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="generic-assertions-array-of-option-constructor"/><a href="#generic-assertions-array-of-option-constructor" class="list-anchor">#</a>
  ...
  Конструктор ожидаемого объекта, например `ExampleClass`, или «обёрнутого» примитивного типа, например `Number`.
