...
Заголовок `Cookie` нельзя переопределить с помощью этого метода. Если значение указано, оно будет проигнорировано, а cookie будет загружен из хранилища cookie браузера. Чтобы задать пользовательские cookie, используйте [BrowserContext.AddCookiesAsync()](/api/class-browsercontext.mdx#browser-context-add-cookies).
:::
