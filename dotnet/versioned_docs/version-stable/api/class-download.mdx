...
Если вам не нужен читаемый поток (readable stream), обычно проще прочитать файл с диска после завершения скачивания. См. [Download.PathAsync()](/api/class-download.mdx#download-path).
:::
