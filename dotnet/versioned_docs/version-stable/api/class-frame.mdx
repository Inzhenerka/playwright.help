...
- `options` `FrameDragAndDropOptions?` *(опционально)*
  ...
  - `SourcePosition` SourcePosition? *(опционально)* <font size="2">Добавлено в: v1.14</font><a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="frame-drag-and-drop-option-source-position"/><a href="#frame-drag-and-drop-option-source-position" class="list-anchor">#</a>
    ...
    Щёлкает по исходному элементу в этой точке относительно верхнего левого угла области внутренних отступов элемента. Если не указано, используется какая‑нибудь видимая точка элемента.
  - `Steps` [int]? (опционально) <font size="2">Добавлено в: v1.57</font><a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="frame-drag-and-drop-option-steps"/><a href="#frame-drag-and-drop-option-steps" class="list-anchor">#</a>
    ...
    По умолчанию — 1. Отправляет `n` интерполированных событий `mousemove`, чтобы представить перемещение между `mousedown` и `mouseup` при перетаскивании. При значении 1 испускает одно событие `mousemove` в конечной точке.
