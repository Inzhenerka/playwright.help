...
Для каждого вложения, загружаемого страницей, генерируется событие [Page.Download](/api/class-page.mdx#page-event-download). Все эти вложения загружаются во временную папку. Вы можете получить URL загрузки, имя файла и поток данных, используя объект [Download] из события.

Вы можете указать, где сохранять загруженные файлы, используя опцию [DownloadsPath](/api/class-browsertype.mdx#browser-type-launch-option-downloads-path) в [BrowserType.LaunchAsync()](/api/class-browsertype.mdx#browser-type-launch).
