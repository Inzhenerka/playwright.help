...
Веб-приложения, которые обрабатывают устаревшие [события касания](https://developer.mozilla.org/en-US/docs/Web/API/Touch_events) для реагирования на жесты, такие как смахивание, сжатие и нажатие, могут быть протестированы путем ручной отправки [TouchEvent](https://developer.mozilla.org/en-US/docs/Web/API/TouchEvent/TouchEvent) на страницу. Примеры ниже демонстрируют, как использовать [Locator.DispatchEventAsync()](/api/class-locator.mdx#locator-dispatch-event) и передавать точки [Touch](https://developer.mozilla.org/en-US/docs/Web/API/Touch) в качестве аргументов.

Обратите внимание: [Locator.DispatchEventAsync()](/api/class-locator.mdx#locator-dispatch-event) не устанавливает свойство [`Event.isTrusted`](https://developer.mozilla.org/en-US/docs/Web/API/Event/isTrusted). Если ваша веб-страница на него опирается, убедитесь, что во время теста вы отключили проверку `isTrusted`.
...
В примере ниже мы эмулируем жест сжатия, то есть две точки касания, которые приближаются друг к другу. Ожидается, что это уменьшит масштаб карты. Приложение, которое тестируется, использует только координаты `clientX/clientY` точек касания, поэтому мы инициализируем только их. В более сложном сценарии вам может понадобиться также установить `pageX/pageY/screenX/screenY`, если ваше приложение их использует.
