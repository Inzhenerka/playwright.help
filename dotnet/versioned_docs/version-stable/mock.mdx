...
#### 
В качестве альтернативы вы также можете записывать HAR-файлы, используя параметр [RecordHarPath](/api/class-browser.mdx#browser-new-context-option-record-har-path) в [Browser.NewContextAsync()](/api/class-browser.mdx#browser-new-context) при создании контекста браузера. Это позволяет захватывать весь сетевой трафик для всего контекста до его закрытия.
