...
### Запуск тестов в разных браузерах: переменная окружения Browser {#run-tests-on-different-browsers-browser-env}
...
### Запуск тестов в разных браузерах: конфигурация запуска {#run-tests-on-different-browsers-launch-configuration}
...
### Запуск тестов с несколькими рабочими потоками: {#run-tests-with-multiple-workers}
...
См. [here](https://xunit.net/docs/running-tests-in-parallel.html) для получения дополнительной информации о запуске тестов в параллельном режиме с xUnit v3.

:::note

xUnit v3 по умолчанию использует алгоритм параллелизма [`conservative`](https://xunit.net/docs/running-tests-in-parallel.html#algorithms).
:::
