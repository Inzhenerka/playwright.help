...
## Инспектор Playwright {#playwright-inspector}
...
- Браузеры запускаются в режиме с графическим интерфейсом
...
Чтобы ускорить процесс отладки, вы можете добавить метод [Page.PauseAsync()](/api/class-page.mdx#page-pause) в ваш тест. Таким образом, вам не придется пошагово проходить каждое действие вашего теста, чтобы добраться до точки, где вы хотите отладить.
...
Во время отладки вам может понадобиться выбрать более устойчивый локатор. Вы можете сделать это, нажав на кнопку **Pick Locator** и наведя курсор на любой элемент в окне браузера. При наведении на элемент вы увидите код, необходимый для нахождения этого элемента, выделенный ниже. Щелчок по элементу в браузере добавит локатор в поле, где вы затем сможете его настроить или скопировать в ваш код.
...
Playwright проанализирует вашу страницу и определит лучший локатор, отдавая приоритет [ролям, тексту и тестовым идентификаторам локаторов](./locators.mdx). Если Playwright найдет несколько элементов, соответствующих локатору, он улучшит локатор, чтобы сделать его устойчивым и уникально идентифицировать целевой элемент, так что вам не придется беспокоиться о сбоях тестов из-за локаторов.

### Логи доступности {#actionability-logs}
...
## Просмотр трассировки {#trace-viewer}

[Trace Viewer](/trace-viewer.mdx) Playwright — это графический инструмент, который позволяет вам исследовать записанные трассы Playwright ваших тестов. Вы можете перемещаться вперед и назад через каждое действие на левой стороне и визуально видеть, что происходило во время действия. В центре экрана вы можете видеть снимок DOM для действия. На правой стороне вы можете видеть детали действия, такие как время, параметры, возвращаемое значение и лог. Вы также можете исследовать сообщения консоли, сетевые запросы и исходный код.
...
Чтобы отладить ваши тесты с использованием инструментов разработчика браузера, начните с установки точки останова в вашем тесте, чтобы приостановить выполнение, используя метод [Page.PauseAsync()](/api/class-page.mdx#page-pause).
...
## Подробные логи API {#verbose-api-logs}
...
## Режим с графическим интерфейсом {#headed-mode}
