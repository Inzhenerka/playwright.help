...
| | | [Expect(Locator).ToBeCheckedAsync()](/api/class-locatorassertions.mdx#locator-assertions-to-be-checked) | Флажок отмечен |
| | | [Expect(Locator).ToBeDisabledAsync()](/api/class-locatorassertions.mdx#locator-assertions-to-be-disabled) | Элемент отключён |
| | | [Expect(Locator).ToBeEditableAsync()](/api/class-locatorassertions.mdx#locator-assertions-to-be-editable) | Элемент доступен для редактирования |
...
| | | [Expect(Locator).ToBeInViewportAsync()](/api/class-locatorassertions.mdx#locator-assertions-to-be-in-viewport) | Элемент пересекает область просмотра |
| | | [Expect(Locator).ToBeVisibleAsync()](/api/class-locatorassertions.mdx#locator-assertions-to-be-visible) | Элемент виден |
| | | [Expect(Locator).ToContainClassAsync()](/api/class-locatorassertions.mdx#locator-assertions-to-contain-class) | Элемент содержит указанные CSS‑классы |
...
| | | [Expect(Locator).ToHaveAccessibleDescriptionAsync()](/api/class-locatorassertions.mdx#locator-assertions-to-have-accessible-description) | Элемент имеет соответствующее [accessible description](https://w3c.github.io/accname/#dfn-accessible-description) |
| | | [Expect(Locator).ToHaveAccessibleNameAsync()](/api/class-locatorassertions.mdx#locator-assertions-to-have-accessible-name) | Элемент имеет соответствующее [accessible name](https://w3c.github.io/accname/#dfn-accessible-name) |
...
| | | [Expect(Locator).ToHaveClassAsync()](/api/class-locatorassertions.mdx#locator-assertions-to-have-class) | Элемент имеет свойство class |
| | | [Expect(Locator).ToHaveCountAsync()](/api/class-locatorassertions.mdx#locator-assertions-to-have-count) | Список содержит точное количество дочерних элементов |
| | | [Expect(Locator).ToHaveCSSAsync()](/api/class-locatorassertions.mdx#locator-assertions-to-have-css) | Элемент имеет CSS‑свойство |
| | | [Expect(Locator).ToHaveIdAsync()](/api/class-locatorassertions.mdx#locator-assertions-to-have-id) | Элемент имеет идентификатор ID |
| | | [Expect(Locator).ToHaveJSPropertyAsync()](/api/class-locatorassertions.mdx#locator-assertions-to-have-js-property) | Элемент имеет свойство JavaScript |
| | | [Expect(Locator).ToHaveRoleAsync()](/api/class-locatorassertions.mdx#locator-assertions-to-have-role) | Элемент имеет определённую [ARIA role](https://www.w3.org/TR/wai-aria-1.2/#roles) |
...
| | | [Expect(Locator).ToHaveValuesAsync()](/api/class-locatorassertions.mdx#locator-assertions-to-have-values) | Элемент select имеет выбранные опции |
| | | [Expect(Locator).ToMatchAriaSnapshotAsync()](/api/class-locatorassertions.mdx#locator-assertions-to-match-aria-snapshot) | Элемент соответствует предоставленному снимку Aria |
