...
## Тестирование утверждений против тестирования снимков {#assertion-testing-vs-snapshot-testing}
...
*снимок aria для частичного совпадения*
...
Свойство `/children` можно использовать для управления тем, как сопоставляются дочерние элементы:
- `contain` (по умолчанию): Совпадает, если все указанные дочерние элементы присутствуют в заданном порядке
- `equal`: Совпадает, если дочерние элементы точно соответствуют указанному списку в заданном порядке
- `deep-equal`: Совпадает, если дочерние элементы точно соответствуют указанному списку в заданном порядке, включая вложенные элементы
...
*снимок aria завершится сбоем из‑за отсутствия Feature C в шаблоне*
...
### Сопоставление с использованием регулярных выражений {#matching-with-regular-expressions}
